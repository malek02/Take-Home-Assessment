          ...
          <FieldArray name="selectedImage">
            {({ push, remove }) =>
              mediaAssets?.map(asset => {
                const isSelected = values?.values?.selectedImage?.includes(asset.url);
                ...
                return (
                  <div key={asset.url} className={` relative w-[220px] h-[200px] hover:opacity-60 hover:cursor-pointer`}>
          ...
          <FieldArray name="selectedVideo">
            {({ push, remove }) =>
              videoAssets?.map(asset => {
                const isSelected = values?.values?.selectedVideo?.includes(asset.url);
                ...
                return (
                  <div key={asset.url} className={` relative w-[220px] h-[200px]  hover:cursor-pointer`}>
                    <div className="absolute top-1 right-1">
                      <Checkbox
                        value={asset.url.toString()}
                        checked={isSelected}
